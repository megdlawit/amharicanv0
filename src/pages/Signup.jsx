...
import { Eye, EyeOff, UserPlus, Phone, Mail, ArrowLeft } from 'lucide-react'
...
export default function Signup() {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    // Create profile row for phone users
    await supabase.from('users').upsert({
      id: data.user.id, phone: fullPhone, name: name.trim(),
      xp: 0, streak_count: 0, role: 'user', level: 'A1', goal_minutes: 10,
    }, { onConflict: 'id' })
    navigate('/onboarding')
  }
    ...
    <div className="min-h-screen bg-stone-50 flex items-center justify-center px-5">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="text-[26px] font-bold text-stone-900 tracking-tight">Create your account</h1>
          <p className="text-stone-400 text-sm mt-1">Free forever. Start learning today.</p>
        </div>
        ...
        <div className="card p-7">
          ...
          <div className="relative mb-5">
            ...
            <div className="relative flex justify-center">
              <span className="bg-white px-3 text-[12px] text-stone-400">or sign up with</span>
            </div>
          </div>
          ...
          {mode === 'email' && (
            <form onSubmit={handleSignup} className="space-y-3">
              <div>
                <label className="block text-[13px] font-semibold text-stone-600 mb-1.5">Full Name</label>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="tel" required value={phone} onChange={e => setPhone(e.target.value)}
                    className="input pl-10" placeholder="+251 911 123 456" autoComplete="tel" />
                </div>
                <p className="text-[11px] text-stone-400 mt-1.5">Include country code (e.g. +251 for Ethiopia, +1 for US)</p>
              </div>
              <button type="submit" disabled={loading} className="btn-primary w-full flex items-center justify-center gap-2">
                {loading ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" />
                  : <><UserPlus size={16} /> Send Verification Code</>}
              </button>
            </form>
          )}
          ...
          {mode === 'phone' && otpSent && (
            <form onSubmit={handleVerifyOtp} className="space-y-3">
              ...
              <button type="submit" disabled={loading || otp.length < 6}
                ...
                {loading ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" />
                  : <><UserPlus size={16} /> Verify & Create Account</>}
              </button>
